...
def find_col(df, *cands):
    ...
    for cand in cands:
        ...
        if cand.lower() in lows: return lows[cand.lower()]
    for c in df.columns:
        name = str(c).lower()
        if any(cand.lower() in name for cand in cands): return c
    return None

def build_mapping(df):
    return {
        "adresse": find_col(df, "Adresse", "Adresse complète"),
        "lat":     find_col(df, "Latitude", "Lat", "AI"),
        "lon":     find_col(df, "Longitude", "Lon", "Lng", "Long", "AJ"),
        "actif":   find_col(df, "Actif", "Active", "AO"),
        "ref":     find_col(df, "Référence annonce", "Référence", "AM"),
        "gmap":    find_col(df, "Lien Google Maps", "Google Maps", "Maps", "H"),
    }
...
def extract_lat_lon_from_gmap(url: str):
    if not isinstance(url, str) or not url.strip(): return (None, None)
    url = url.strip()
    ...
    if "ll" in qs:
        try:
            la, lo = qs["ll"][0].split(","); return float(la), float(lo)
        except: pass
    if "q" in qs:
    ...
    if "center" in qs:
        try:
            la, lo = qs["center"][0].split(","); return float(la), float(lo)
        except: pass
...
def main():
    ...
    # ---- Centre France ----
    FR_LAT, FR_LON, FR_ZOOM = 46.603354, 1.888334, 5.3

    # ---- Basemap: OSM Mapnik (Leaflet Providers), with CORS allowed ----
    basemap = pdk.Layer(
        "TileLayer",
        data="https://tile.openstreetmap.org/{z}/{x}/{y}.png",  # pas de {s} pour éliminer l'ambiguïté
        min_zoom=0, max_zoom=19, tile_size=256,
        opacity=1.0
    )

    # Pins bleus + référence
    ref_col = mapcols.get("ref")
    if ref_col and ref_col in df_valid.columns:
    ...
    pins = pdk.Layer(
        ...
        get_radius=40, radius_min_pixels=8, radius_max_pixels=30,
    ...
    view_state = pdk.ViewState(longitude=FR_LON, latitude=FR_LAT, zoom=FR_ZOOM)

    # IMPORTANT: désactiver totalement le fond CARTO de pydeck
    deck = pdk.Deck(
        layers=[basemap, pins, labels],
        initial_view_state=view_state,
        tooltip=None,
        map_style=None,
        map_provider=None,                 # <- empêche l'ajout du fond CARTO par défaut
        parameters={"cull": True},
    )

    # Pour certains environnements, il faut expliciter le CORS des images
    deck._js_use_custom_draw = True  # no-op mais empêche d'injecter des styles mapbox
