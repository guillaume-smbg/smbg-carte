...
st.set_page_config(page_title='SMBG Carte (Leaflet + Drawer)', layout='wide')
...
def build_mapping(df):
    ...
    slice_cols = colnames[start_idx:end_idx+1] if len(colnames) > end_idx else colnames[start_idx:]
...
def build_geojson(df_valid: pd.DataFrame, ref_col: str | None, range_cols):
    feats = []
...
def render_map_with_drawer(df_valid: pd.DataFrame, range_cols, ref_col: str | None):
    ...
    <style>
      ...
      .smbg-pin {{
        background:{LOGO_BLUE}; color:#fff; border:2px solid #fff; width:28px; height:28px;
        line-height:28px; border-radius:50%; text-align:center; font-size:11px; font-weight:700;
      }}
    ...
    <script>
      ...
      function openDrawerWithProps(props) {{
        ...
        COL_ORDER.forEach(col => {{
          const raw = props[col];
          if (isMeaningful(raw)) {{
            body.insertAdjacentHTML('beforeend', buildRowHTML(col, String(raw)));
          }}
        }});
...
def main():
    ...
    html_str = render_map_with_drawer(df_valid, range_cols, ref_col)
