...
# -------------------------------------------------
# CSS GLOBAL (sidebar bleue, panneau droit fixe, scroll interne)
# -------------------------------------------------
...
<style>
  ...
  .right-panel-box {{
    ...
    background:#fff;
    height: calc(100vh - 16px); /* prend quasi toute la hauteur viewport */
    display:flex;
    flex-direction:column;
    font-family: system-ui, sans-serif;
    overflow: hidden; /* pas de scroll global, uniquement interne */
  }}
  ...
  .panel-body {{
    flex:1;
    overflow-y:auto;   /* scroll interne ici uniquement */
  ...
  /* indentation visuelle des départements */
  .dept-row {{
    display: flex;
    flex-direction: row;
    align-items: center;
...
# Fallback local si EXCEL_URL pas dispo
...
def load_excel() -> pd.DataFrame:
    """
    Charge l'Excel depuis EXCEL_URL (secrets/env) ou local.
    """
...
def normalize_bool(val):
    """Convertit 'oui' / 1 / True en booléen True/False."""
...
def norm_txt(x: str) -> str:
    """Minuscule, sans accents, espaces réduits. Sert pour matcher."""
    if x is None:
        return ""
    s = str(x).strip().lower()
...
def sanitize_value(v):
    """Pour l'affichage du tableau détail : remplace -, /, '' par vide."""
    if v is None:
        return ""
    s = str(v).strip()
...
def find_col(df: pd.DataFrame, *candidates) -> str:
    """
    Essaie d'identifier une colonne du df qui correspond à un nom attendu.
    On matche sur version normalisée de l'intitulé.
    """
    def _norm(x: str) -> str:
        if x is None:
            return ""
...
def to_number(value) -> Optional[float]:
    ...
    Retourne None si pas de nombre.
    """
    if value is None:
        return None
    s = str(value).strip()
...
def clean_latlon_series(series: pd.Series) -> pd.Series:
    """Latitude/Longitude -> float en gérant les virgules."""
    return (
        series.astype(str)
        .str.strip()
...
def reset_filters_defaults():
    """
    Supprime tous les filtres dans session_state puis rerun.
    """
...
def build_lots_table(df_ref: pd.DataFrame) -> pd.DataFrame:
    """
    Prépare le tableau affiché dans le panneau droit.
    On tente de prendre les colonnes G→AH (6:34),
    sinon on prend tout.
    On nettoie '-' '/' ''.
    """
...
def anti_overlap_positions(n: int, base_lat: float, base_lon: float) -> List[Tuple[float, float]]:
    """
    Si plusieurs annonces ont EXACTEMENT les mêmes coords,
    on les décale un peu en cercle pour éviter superposition visuelle.
    """
    if n <= 1:
...
def overlaps(a_min, a_max, b_min, b_max):
    ...
    Si info manquante -> True (on garde l'annonce).
    """
    if (
        a_min is None
        or a_max is None
...
def normalize_ref_str(x: str) -> str:
    """
    Supprime le '.0' final si c'est un entier genre '22.0'.
    """
    x = str(x).strip()
    if re.match(r"^\d+\.0+$", x):
        return x.split(".")[0]
...
def nearest_ref_from_click(lat_clicked: float,
                           lon_clicked: float,
                           click_registry: Dict[Tuple[float,float], str],
                           tolerance: float = 0.001) -> str:
    """
    Cherche dans click_registry (coords -> ref) la clé la plus proche
    des coords cliquées. tolerance ~0.001° ~ 100m.
    Retourne "" (vide) si aucune clé proche.
    """
    best_ref = ""
    best_dist = None

    for (plat, plon), refv in click_registry.items():
        dlat = plat - lat_clicked
        dlon = plon - lon_clicked
    ...
    # Si la meilleure distance est vraiment loin (> tolerance), on considère clic vide
    if math.sqrt(best_dist) > tolerance:
        return ""
...
def render_right_panel(selected_ref: Optional[str],
    ...
    if not selected_ref:
        ...
        st.markdown(
            '<div class="panel-body"><p>Sélectionnez une annonce sur la carte.</p></div>',
            unsafe_allow_html=True
        )
        st.markdown('</div>', unsafe_allow_html=True)
        return

    # Extraire les lignes qui matchent la ref sélectionnée
...
def main():
    ...
    # 2. Identifier les colonnes clefs
    ...
    if not col_lat or not col_lon or not col_ref:
        st.error("Colonnes Latitude / Longitude / Référence annonce introuvables dans l'Excel.")
        st.stop()
    ...
    # catégorisation texte normalisée pour les filtres
    ...
    # on ne garde que les lignes actives et géocodées
    ...
    with st.sidebar:
        ...
        # Région / Département regroupés visuellement
        if col_region and col_dept:
            with st.expander("Région / Département", expanded=True):
                regions = sorted([
                    r for r in lots_working[col_region]
                ...
                for reg in regions:
                    ...
                    if st.checkbox(reg, key=f"reg_{reg}"):
                        ...
                        # afficher les départements indentés
                        for dep in deps:
                            cpad, cbox = st.columns([1,10])
                            with cpad:
                                st.write("")  # sert juste à créer le retrait visuel
                            with cbox:
                                if st.checkbox(dep, key=f"dep_{reg}_{dep}"):
                                    selected_deps.append(dep)

                # appliquer les filtres géo
                if selected_regions:
        ...
        if col_typo:
            ...
            typos_raw = sorted([
                t for t in lots_working[col_typo]
                .dropna().astype(str).unique()
                if t not in ["-", "/", ""]
            ])
            chosen_norm = []
        ...
        if col_empl:
            ...
            vals = [
                e for e in lots_working[col_empl]
                .dropna().astype(str).unique()
                if e not in ["-", "/", ""]
            ]
    ...
    # 5. Agréger par référence annonce => 1 pin = 1 annonce
    def ref_agg(group: pd.DataFrame) -> pd.Series:
        out_d = {}
        # coords moyennes
        out_d["_lat"] = group["_lat"].mean()
        out_d["_lon"] = group["_lon"].mean()

        # plage surfaces
        if col_surface:
            svals = clean_numeric_series(group[col_surface]).dropna()
            out_d["surf_min"] = float(svals.min()) if not svals.empty else None
            out_d["surf_max"] = float(svals.max()) if not svals.empty else None
        else:
            out_d["surf_min"] = None
            out_d["surf_max"] = None
        ...
        # plage loyers
        if col_loyer:
            lvals = clean_numeric_series(group[col_loyer]).dropna()
            out_d["loyer_min"] = float(lvals.min()) if not lvals.empty else None
            out_d["loyer_max"] = float(lvals.max()) if not lvals.empty else None
        else:
        ...
        ref_val = normalize_ref_str(ref_val)
    ...
    if lmin is not None and lmax is not None and "loyer_range" in st.session_state:
        ...
        refs = refs[
            refs.apply(lambda r: overlaps(r.get("loyer_min"), r.get("loyer_max"), rmin, rmax), axis=1)
        ]

    if refs.empty:
        # si plus rien, on affiche quand même la structure 2 colonnes
    ...
    # anti-overlap pour refs partageant la même coordonnée brute
    ...
    for (_, _), grp in refs.groupby(["_lat_r", "_lon_r"], sort=False):
        ...
        for (lat, lon), (_, row) in zip(coords, grp.iterrows()):
            rr = row.copy()
            rr["_lat_plot"] = lat
            rr["_lon_plot"] = lon
    ...
    # état de la ref sélectionnée (pour panneau droit)
    if "selected_ref" not in st.session_state:
        st.session_state["selected_ref"] = None

    # 6. Mise en page principale : carte à gauche / panneau droit à droite
    left_col, right_col = st.columns([1, 0.28], gap="small")
    ...
    with left_col:
        ...
        for _, r in plot_df.iterrows():
            ...
            raw_label = normalize_ref_str(raw_label)

            icon = folium.DivIcon(
                html=f'<div style="{css_marker}">{raw_label}</div>'
            )

            # Enregistre la ref pour ces coords
            key = (round(lat, 6), round(lon, 6))
            click_registry[key] = raw_label

            # IMPORTANT : ni popup ni tooltip => pas de survol / pas de bulle
            folium.Marker(
                location=[lat, lon],
                icon=icon,
            ).add_to(layer)
        ...
        # hauteur fixe (~800px) pour limiter le scroll global
        map_output = st_folium(m, height=800, width=None, returned_objects=[])

        # Détection du clic utilisateur
        clicked_ref = None
        if isinstance(map_output, dict):
            # on essaie plusieurs structures possibles pour la donnée clic
            loc_info = (
                map_output.get("last_object_clicked")
                or map_output.get("last_object_clicked_marker")
                or map_output.get("last_object_clicked_tooltip")
            )

            if isinstance(loc_info, dict):
                # certains retours sont {"lat":..., "lng":...}
                lat_clicked = loc_info.get("lat")
                lon_clicked = loc_info.get("lng")
                if lat_clicked is None and "lon" in loc_info:
                    lon_clicked = loc_info.get("lon")
                if lat_clicked is not None and lon_clicked is not None:
                    try:
                        lat_clicked = float(lat_clicked)
                        lon_clicked = float(lon_clicked)
                        # On cherche la ref la plus proche (tolérance ~100m)
                        guessed = nearest_ref_from_click(
                            lat_clicked,
                            lon_clicked,
                            click_registry,
                            tolerance=0.001
        ...
        # "" => clic dans le fond de carte => panneau = Aucune sélection
