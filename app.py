...
# --------------------- Column detection (by headers, not letters) ---------------------
...
# --------------------- QC helpers ---------------------
def qc_exclusions(df, mapcols):
    reasons = []
    actif_col = mapcols.get("actif")
    latc, lonc, addrc = mapcols["lat"], mapcols["lon"], mapcols["adresse"]
    for i, row in df.iterrows():
        ok = True
        if actif_col is not None and not normalize_bool(row.get(actif_col)):
            reasons.append((i, "Inactif (Actif != oui)"))
            ok = False
        lat = pd.to_numeric(row.get(latc), errors="coerce")
        lon = pd.to_numeric(row.get(lonc), errors="coerce")
        if ok and (pd.isna(lat) or pd.isna(lon)):
            reasons.append((i, "Coordonnées manquantes après géocodage"))
            ok = False
        if ok and not (-90 <= float(lat) <= 90 and -180 <= float(lon) <= 180):
            reasons.append((i, "Coordonnées hors limites"))
            ok = False
        if ok and (not row.get(addrc) or str(row.get(addrc)).strip() == ""):
            reasons.append((i, "Adresse vide"))
            ok = False
    return reasons
...
def main():
    ...
    # Filtre Actif (par défaut tout actif si colonne absente)
    ...
    # Appliquer filtres et nettoyages
    ...
    if df_valid.empty:
        ...
        layer = pdk.Layer(
            ...
            get_radius=40,                  # rayon en mètres
            radius_min_pixels=8,            # 👈 assure la visibilité dès le zoom France
    ...
    # QC panel — pour expliquer 23 vs 25
    excl = qc_exclusions(df, mapcols)
    with st.expander("Debug (colonnes détectées & contrôle qualité)", expanded=False):
        st.write(mapcols)
        st.write(f"Lignes totales dans l'Excel : {total_rows}")
        st.write(f"Points affichés : {len(df_valid)}")
        if excl:
            idxs, reasons = zip(*excl)
            qc_df = df.loc[list(idxs), [c for c in [mapcols.get('ref'), mapcols.get('adresse'), mapcols.get('actif'), mapcols.get('lat'), mapcols.get('lon')] if c]].copy()
            qc_df["Raison exclusion"] = list(reasons)
            st.dataframe(qc_df)
        else:
            st.write("Aucune exclusion détectée.")
