...
def main():
    ...
    # --- Centering logic ---
    # If points exist, center on their median (robust to outliers), else fallback to France center.
    if not df_valid.empty:
        center_lat = float(df_valid["_lat"].median())
        center_lon = float(df_valid["_lon"].median())
    else:
        center_lat, center_lon = 46.6, 2.2  # France metro approx
    # Optional: small zoom bias for France
    initial_zoom = 5.2

    # Mapnik tiles (official OSM)
    basemap = pdk.Layer(
        "TileLayer",
    ...
    view_state = pdk.ViewState(longitude=center_lon, latitude=center_lat, zoom=initial_zoom)
