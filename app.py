...
def _github_folder_to_api(url: str) -> str:
    # Convert to GitHub Contents API:
...
def _fetch_first_excel_from_github_folder(folder_url: str) -> bytes:
    ...
    # Cherche .xlsx en priorité puis .xls
    excel_items = [e for e in entries if e.get('type') == 'file' and (e.get('name','').lower().endswith('.xlsx') or e.get('name','').lower().endswith('.xls'))]
    if not excel_items:
        raise FileNotFoundError("Aucun fichier Excel (.xlsx/.xls) trouvé dans ce dossier GitHub.")
    # Prend le premier .xlsx
...
def load_excel() -> pd.DataFrame:
    ...
    if not os.path.exists(path):
        st.error(
            ...
            "• OU définissez EXCEL_URL (Secrets/variable d'env) vers un **fichier** raw **ou un dossier GitHub** contenant un .xlsx."
        )
        st.stop()
...
- **Option A (local)** : placez votre fichier à `data/Liste_des_lots.xlsx` dans le repo.
- **Option B (secret `EXCEL_URL`)** :
  - Mettez soit **un lien RAW direct** vers un `.xlsx`,
  - soit **l'URL du *dossier* GitHub** (ex: `https://github.com/owner/repo/tree/main/data`) — l'app détectera automatiquement le premier `.xlsx`.
    \"\"\")
