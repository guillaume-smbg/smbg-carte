...
# ================== PAGE LAYOUT ==================
st.set_page_config(page_title="SMBG Carte (Leaflet Mapnik - Auto)", layout="wide")
...
# No cache: read fresh on each page load
...
# ================== GEOCODING (auto-healing) ==================
def clean_address(a: str) -> str:
    a = re.sub(r'\s+', ' ', str(a)).strip()     # remplace \n etc.
    a = re.sub(r'\s*-\s*', ' ', a)              # "34470 - Pérols" -> "34470 Pérols"
...
# Success-only cache (session) - do not cache failures
...
def best_effort_geocode(address: str, gmap_url: str):
