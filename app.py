...
def ensure_latlon(df: pd.DataFrame):
    ...
    adresse = df["G"].fillna("").astype(str) if "G" in df.columns else pd.Series([""] * len(df))
    # Try to find a 'Ville' header, case-insensitive
    ville_col = None
    for c in df.columns:
        if str(c).strip().lower() == "ville":
            ville_col = c
            break
    if ville_col:
        ville = df[ville_col].fillna("").astype(str)
        full_addr = adresse.str.cat(ville, sep=", ", na_rep="")
    else:
        full_addr = adresse

    email = st.secrets.get("NOMINATIM_EMAIL", os.environ.get("NOMINATIM_EMAIL", ""))

    # Need geocode when AI/AJ are NaN OR empty strings OR not numbers
    lat_num = pd.to_numeric(df["AI"], errors="coerce")
    lon_num = pd.to_numeric(df["AJ"], errors="coerce")
    need_geo = lat_num.isna() | lon_num.isna()

    to_geo = df[need_geo].copy()
    if not to_geo.empty:
        # Optional manual trigger: if user clicks the button, force geocode now; else do it automatically too.
        force = st.sidebar.button("Forcer le géocodage maintenant", type="primary")
        if force or True:
            with st.spinner(f"Géocodage des adresses manquantes ({len(to_geo)})…"):
                for idx, row in to_geo.iterrows():
                    addr = full_addr.loc[idx]
...
def main():
    ...
    layer = pdk.Layer("ScatterplotLayer", data=df, get_position=["_lon", "_lat"], get_radius=20, get_fill_color=LOGO_BLUE_RGB, pickable=True)
