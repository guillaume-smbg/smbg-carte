...
def format_value(value, unit=""):
    ...
    s=str(value).strip()
    # Règle de masquage : vide, néant, -, /, etc.
    if s.lower() in ("n/a","nan","","none","néant","-","/"): return ""
    try:
        # Conversion en nombre, gestion des virgules/espaces et masquage si 0
        ...
        return s # Retourne la chaîne brute si la conversion échoue

def jitter_group(df, lat_col, lon_col, base_radius_m=12.0, step_m=4.0):
    # ANCIENNE FONCTION - IGNORÉE selon la consigne "Pas de Jitter"
    return df.copy() 
...
# ===== CSS global (Layout 3 zones, Rétractation, 275px) =====
def logo_base64():
...
/* Panneau droit réservé : Ajusté à 275px + 20px de marge */
...
/* Panneau droit RÉTRACTABLE (details-panel-container remplace .details-panel) */
...
/* Classe pour déplier (apparait au clic pin) */
.details-panel-open {{
    width: {DETAIL_PANEL_WIDTH};
    padding:16px;
}}
/* Le contenu est masqué tant que le panneau n'a pas la largeur définie */
...
.maps-button {{
  ...
  text-decoration:none; display:inline-block; /* Rendu du <a> */
}}
.details-panel-container table {{ width:100%; border-collapse:collapse; font-size:13px; }}
.details-panel-container tr {{ border-bottom:1px solid #304f65; }} /* Couleur ajustée */
.details-panel-container td {{ padding:6px 0; max-width:50%; overflow-wrap:break-word; }}
</style>
""", unsafe_allow_html=True)

# ===== SIDEBAR (Le reste de la sidebar reste inchangé, mais utilise SIDEBAR_WIDTH) =====
with st.sidebar:
    ...
    l_step = 1000 if loyer_max > 50000 else 100 # Ajout d'une meilleure gestion de step
...
if selected_regions or selected_depts_global:
    ...
    if selected_depts_global:
        # On ajoute les départements sélectionnés à l'union globale
        # Note: La logique imbriquée assure déjà une bonne couverture
        pass 
...
# ===== CARTE (clic strict sur pin; pas de popup visible; PAS DE JITTER) =====
pins_df = f.copy()
...
def add_pin(lat, lon, label, ref_value):
    ...
    # 1. Marker + popup technique (masqué en CSS) pour véhiculer l’event
    folium.Marker(
        location=[lat,lon],
    ...
    # 2. Cible cliquable (CircleMarker) exactement sur le pin (masqué en CSS)
    folium.CircleMarker(
...
if not pins_df.empty:
    ...
    # **PAS DE JITTER : on utilise LAT_COL et LON_COL directement**
    for _,r in pins_df.iterrows():
        add_pin(float(r[LAT_COL]), float(r[LON_COL]), r["__ref_display__"], r[REF_COL])

# écoute l’objet cliqué ET les coordonnées cliquées
map_output = st_folium(
    m, 
    height=MAP_HEIGHT, 
    width="100%", 
    returned_objects=["last_object_clicked", "last_clicked"], # **AJOUT DE last_clicked pour la fermeture**
    key="map"
)
...
if map_output and map_output.get("last_object_clicked"):
    ...
    ref_guess=None
    # Tente d'extraire la référence cachée dans le popup technique
    for k in ("popup","popup_html"):
        if k in obj and obj[k]:
            ...
            break
    if ref_guess:
        st.session_state["selected_ref"]=ref_guess # Clic sur Pin : Ouvre le panneau
        ref_clicked = ref_guess # Marque l'événement

# Règle 2: Clic sur la carte (hors pin)
# Si on a un événement de clic sur la carte ET qu'aucun pin n'a été cliqué
elif map_output and map_output.get("last_clicked") and not ref_clicked:
    st.session_state["selected_ref"] = None # Clic sur Carte : Ferme le panneau

# ===== VOLET DROIT (Rétractable 275px) =====
sel_ref = st.session_state.get("selected_ref")
is_open = False # État du panneau
html_content = []
...
if sel_ref:
    ...
    if not row.empty:
        ...
        for champ in cols_slice:
            ...
            # --- Lien Google Maps (Bouton) ---
            # On cherche par nom de colonne (plus robuste que l'index H/INDEX_START+1)
            ...
            # --- Autres champs (Formatage et Masquage) ---
...
# Affichage du panneau (état 'open' ou 'closed' géré par la classe CSS)
