...
"""
st.markdown(_CSS.replace("__BLUE__", LOGO_BLUE).replace("__COPPER__", COPPER), unsafe_allow_html=True)
...
def render_drawer(row: pd.Series):
    ...
    gm = row.get("Lien Google Maps", "")
    display_cols = slice_g_to_ah(row.to_frame().T)

    st.markdown('<div class="smbg-drawer open">', unsafe_allow_html=True)
...
def main():
    ...
    for _, r in scoped.iterrows():
        ...
        # Put the ref in the popup to catch clicks with st_folium
    ...
    out = st_folium(m, width=None, height=950, returned_objects=[])

    st.markdown('</div>', unsafe_allow_html=True)

    # Open right drawer when a pin was clicked (detect popup text)
    ref_clicked = None
